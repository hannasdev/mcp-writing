...
| Sidecar migration/readMeta | `src/sync/sync.js:513`, `src/sync/sync.js:525` | Sidecar writes | Writes generated sidecar from frontmatter when writable and writes metadata sidecars directly. | Sidecar helper scoped to validated prose path inside sync root. | Broad sync tests cover sidecar generation; missing targeted symlink-target rejection. |
...
| Prose edit commit | `src/tools/editing.js:500` | Authored prose | Writes accepted proposal content to `proposal.scene_file_path` after stale diagnostics and snapshot creation. | Authored-prose helper that revalidates scene path inside sync root and rejects non-regular/symlink targets before write. | `src/test/integration/editing.test.mjs`; missing targeted invalid path/symlink write tests. |
| Metadata reference updates | `src/tools/metadata.js:59` | Metadata/frontmatter write | Writes updated frontmatter to a provided character/reference file path after metadata reads. | Metadata helper scoped to validated sync-root document path and regular-file target. | `src/test/integration/metadata.test.mjs`; missing focused symlink-target tests. |
| World entity creation | `src/core/helpers.js:288`, `src/core/helpers.js:295`, `src/core/helpers.js:301`, `src/core/helpers.js:306` | World entity prose/meta/arc | Creates entity directory, prose sheet, optional character arc, and metadata. | World-entity helper scoped to sync-root world directory with separate prose/meta write choices. | Metadata/reference tool tests cover behavior; missing symlink ancestor and partial-write handling tests. |
...
- prose, sidecar, metadata, world-entity, styleguide config, and AI boot writes
  should each get at least one symlink-target rejection test when moved to
  artifact-aware helpers;
